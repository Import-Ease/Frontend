...
import {
  ...
  PencilIcon, LockIcon, ClockIcon, DollarSignIcon, GlobeIcon,
} from '../components/Icons';
import { fetchShipments, fetchTotalCost, deleteMyAccount } from '../services/api';
import { clearAuthState, getSavedAccounts, switchAccount, logoutCurrent, removeSavedAccount, StoredAccount } from '../services/storage';
...
export default function SettingsScreen() {
  ...
  const role = (globalThis as any).__IMPORT_EASE_ROLE__ as string | undefined;
  ...
  const handleRetry = useCallback(() => {
    setError(null); setLoading(true); loadData();
  }, [loadData]);
  ...
  const pendingPaymentCount = shipments.filter((s) => s.rawStatus === 'PENDING_PAYMENT' || s.rawStatus === 'ORDER_CREATED').length;
  ...
  const handleLogout = async () => {
    ...
    if (saved.length > 0) {
      // return to Login with account list available; login screen handles switching
    }
  ...
  const handleRemoveSavedAccount = (account: StoredAccount) => {
    Alert.alert('Remove account', `Remove ${account.username || account.email || 'this account'} from this device?`, [
      { text: 'Cancel', style: 'cancel' },
      {
        text: 'Remove', style: 'destructive',
        onPress: async () => {
          await removeSavedAccount(account);
  ...
  const confirmDeleteAccount = async () => {
    if (!deletePassword.trim()) { Alert.alert('Password required', 'Please enter your password.'); return; }
    const token = (globalThis as any).__IMPORT_EASE_TOKEN__ as string | undefined;
    if (!token) { Alert.alert('Not authenticated', 'Please log in again.'); return; }
    setDeleting(true);
    try {
      await deleteMyAccount(deletePassword.trim(), token);
      setDeleteModalVisible(false); setDeletePassword('');
      await clearAuthState();
      navigation.getParent<any>()?.reset({ index: 0, routes: [{ name: 'Login' }] });
    } catch (err: any) {
      Alert.alert('Deletion failed', err?.message || 'Wrong password.');
  ...
  return (
    <SafeAreaView style={[s.safe, { backgroundColor: colors.background }]} edges={['top']}>
      <ScrollView contentContainerStyle={s.content} showsVerticalScrollIndicator={false}>
        ...
        <View style={[s.section, { backgroundColor: colors.card }]}>
          <View style={[s.sectionHeader, { borderBottomColor: colors.border }]}>
            <View style={[s.sectionIconWrap, { backgroundColor: colors.orangeDim }]}>
              <UserIcon size={14} color={colors.orange} />
            </View>
            <Text style={[s.sectionTitle, { color: colors.orange }]}>Profile</Text>
          </View>
          ...
          <TouchableOpacity
            ...
            activeOpacity={0.7}
            onPress={() => {
              if (isSupplier) navigation.navigate('SupplierProfile' as any);
            }}
          >
            ...
            <View style={{ flex: 1 }}>
              ...
              <Text style={[s.rowValue, { color: colors.muted }]}>{email || 'No email'}</Text>
            </View>
            {isSupplier && (
              <View style={{ flexDirection: 'row', alignItems: 'center', gap: 4 }}>
                <PencilIcon size={14} color={colors.cobalt} />
                <Text style={[s.rowHint, { color: colors.cobalt }]}>Edit</Text>
              </View>
            )}
          </TouchableOpacity>
        </View>

        {/* ── REPORTS SECTION (Importer) ────────────── */}
        {!isSupplier && (
          <>
            <View style={[s.sectionHeaderRow]}>
              ...
              <Text style={[s.sectionLabel, { color: colors.green }]}>Reports</Text>
            </View>

            {error ? (
              <View style={[s.emptyCard, { backgroundColor: colors.card }]}>
                <Text style={[s.emptyText, { color: colors.muted }]}>{error}</Text>
                <TouchableOpacity style={[s.retryBtn, { backgroundColor: colors.greenDim }]} onPress={handleRetry} activeOpacity={0.8}>
                  <Text style={[s.retryBtnText, { color: colors.green }]}>Retry</Text>
                </TouchableOpacity>
              </View>
            ) : loading ? (
              ...
              <View style={[s.emptyCard, { backgroundColor: colors.card }]}>
                ...
                <Text style={[s.emptyText, { color: colors.muted }]}>No shipment data yet</Text>
              </View>
            ) : (
              <>
                <View style={s.statsRow}>
                  <StatCard icon={<PackageIcon size={18} color={colors.cobalt} />} val={totalCount} label="Total" color={colors.cobalt} colors={colors} />
                  <StatCard icon={<ClockIcon size={18} color={colors.orange} />} val={pendingPaymentCount} label="Pending" color={colors.orange} colors={colors} />
                  <StatCard icon={<AlertTriangleIcon size={18} color={colors.orange} />} val={alertCount} label="Alerts" color={colors.orange} colors={colors} />
                  <StatCard icon={<CheckCircleIcon size={18} color={colors.green} />} val={deliveredCount} label="Delivered" color={colors.green} colors={colors} />
                </View>
                <View style={[s.costCard, { backgroundColor: colors.card }]}>
                  <Text style={[s.costTitle, { color: colors.muted }]}>Total landed cost</Text>
                  <Text style={[s.costVal, { color: colors.green }]}>{totalCost}</Text>
                </View>
                  ...
                  {[
                    ...
                    <View key={bar.label} style={s.barRow}>
                      ...
                      <View style={s.barTrack}>
                        <View style={[s.barFill, { backgroundColor: bar.color, width: `${(bar.count / maxBar) * 100}%` }]} />
                      </View>
                ...
                <View style={[s.recentCard, { backgroundColor: colors.card }]}>
                  ...
                  {[...shipments].sort((a, b) => {
                    ...
                    <View key={ship.id} style={[s.recentRow, { borderBottomColor: colors.border }]}>
                      <View style={{ flex: 1 }}>
                        <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6, marginBottom: 2 }}>
                          <Text style={[s.recentId, { color: colors.green }]}>{ship.id}</Text>
                          <StatusBadge status={ship.status} label={ship.statusLabel} />
                        </View>
                        <Text style={[s.recentDesc, { color: colors.text }]} numberOfLines={1}>{ship.description}</Text>
                        <Text style={[s.recentMeta, { color: colors.caption }]}>{ship.origin} {'->'} {ship.destination || 'Tema, GH'} . {ship.carrier}</Text>
                      </View>
        ...
        {isSupplier && (
          <>
            ...
            <View style={[s.section, { backgroundColor: colors.card }]}>
              <TouchableOpacity style={s.row} activeOpacity={0.7} onPress={() => navigation.navigate('SalesReport')}>
                <View style={[s.rowIconWrap, { backgroundColor: colors.greenDim }]}>
                  <BarChartIcon size={16} color={colors.green} />
                </View>
                <View style={{ flex: 1 }}>
                  <Text style={[s.rowLabel, { color: colors.text }]}>Sales Reports</Text>
                  <Text style={[s.rowValue, { color: colors.muted }]}>View your sales performance</Text>
                </View>
                <Text style={[s.rowChevron, { color: colors.caption }]}>{'>'}</Text>
              </TouchableOpacity>
              <TouchableOpacity style={s.row} activeOpacity={0.7} onPress={() => navigation.navigate('MyProducts' as any)}>
                <View style={[s.rowIconWrap, { backgroundColor: colors.orangeDim }]}>
                  <DollarSignIcon size={16} color={colors.orange} />
                </View>
                <View style={{ flex: 1 }}>
                  <Text style={[s.rowLabel, { color: colors.text }]}>Revenue Summary</Text>
                  <Text style={[s.rowValue, { color: colors.muted }]}>Dashboard with earnings overview</Text>
                </View>
                <Text style={[s.rowChevron, { color: colors.caption }]}>{'>'}</Text>
              </TouchableOpacity>
              <TouchableOpacity style={s.row} activeOpacity={0.7}>
                <View style={[s.rowIconWrap, { backgroundColor: colors.surfaceAlt }]}>
                  <InfoIcon size={16} color={colors.caption} />
                </View>
                <View style={{ flex: 1 }}>
                  <Text style={[s.rowLabel, { color: colors.text }]}>Export</Text>
                  <Text style={[s.rowValue, { color: colors.muted }]}>Download reports (coming soon)</Text>
                </View>
              </TouchableOpacity>
            </View>
          </>
                ...
                <View key={acc.token || i} style={s.row}>
                  ...
                  {isActive ? (
                    ...
                    <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
                      ...
                      <TouchableOpacity onPress={() => handleRemoveSavedAccount(acc)} activeOpacity={0.7}>
        ...
        <View style={[s.section, { backgroundColor: colors.card }]}>
          <View style={[s.sectionHeader, { borderBottomColor: colors.border }]}>
            <View style={[s.sectionIconWrap, { backgroundColor: colors.navyDim }]}>
              {mode === 'dark' ? <MoonIcon size={14} color={colors.navy} /> : <SunIcon size={14} color={colors.navy} />}
            </View>
            <Text style={[s.sectionTitle, { color: colors.navy }]}>Appearance</Text>
          </View>

          <View style={s.row}>
            <View style={[s.rowIconWrap, { backgroundColor: colors.navyDim }]}>
              {mode === 'dark' ? <MoonIcon size={16} color={colors.navy} /> : <SunIcon size={16} color={colors.navy} />}
            </View>
            <View style={{ flex: 1 }}>
              <Text style={[s.rowLabel, { color: colors.text }]}>Theme</Text>
              <Text style={[s.rowValue, { color: colors.muted }]}>{mode === 'dark' ? 'Dark' : mode === 'light' ? 'Light' : 'System'}</Text>
            </View>
            <View style={{ flexDirection: 'row', gap: 6 }}>
              {(['light', 'dark', 'system'] as const).map((opt) => (
                <TouchableOpacity key={opt} style={[s.themeChip, { backgroundColor: mode === opt ? colors.navy : colors.surfaceAlt }]} onPress={() => setMode(opt)} activeOpacity={0.8}>
                  <Text style={[s.themeChipText, { color: mode === opt ? '#FFFFFF' : colors.muted }]}>
        ...
        <View style={[s.section, { backgroundColor: colors.card }]}>
          <View style={[s.sectionHeader, { borderBottomColor: colors.border }]}>
            <View style={[s.sectionIconWrap, { backgroundColor: colors.cobaltDim }]}>
              <LockIcon size={14} color={colors.cobalt} />
            </View>
            <Text style={[s.sectionTitle, { color: colors.cobalt }]}>Security</Text>
          </View>

          <TouchableOpacity style={s.row} activeOpacity={0.7} onPress={() => navigation.navigate('ChangePassword')}>
            <View style={[s.rowIconWrap, { backgroundColor: colors.cobaltDim }]}>
              <LockIcon size={16} color={colors.cobalt} />
            </View>
            <View style={{ flex: 1 }}>
              <Text style={[s.rowLabel, { color: colors.text }]}>Change Password</Text>
              <Text style={[s.rowValue, { color: colors.muted }]}>Update your account password</Text>
            </View>
            <Text style={[s.rowChevron, { color: colors.caption }]}>{'>'}</Text>
          </TouchableOpacity>
        </View>
        ...
        <View style={[s.section, { backgroundColor: colors.card }]}>
          <View style={[s.sectionHeader, { borderBottomColor: colors.border }]}>
            <View style={[s.sectionIconWrap, { backgroundColor: colors.skyBlueDim }]}>
              <InfoIcon size={14} color={colors.skyBlue} />
            </View>
            <Text style={[s.sectionTitle, { color: colors.skyBlue }]}>About</Text>
          </View>

          <View style={s.row}>
            <View style={[s.rowIconWrap, { backgroundColor: colors.skyBlueDim }]}>
              <ShieldIcon size={16} color={colors.skyBlue} />
            </View>
            <View style={{ flex: 1 }}>
          ...
          <View style={s.row}>
            <View style={[s.rowIconWrap, { backgroundColor: colors.greenDim }]}>
              <GlobeIcon size={16} color={colors.green} />
            </View>
            <View style={{ flex: 1 }}>
        ...
        <View style={{ marginTop: Space.lg }}>
          <TouchableOpacity style={[s.logoutBtn, { backgroundColor: colors.card, borderColor: '#D94452' }]} onPress={handleLogout} activeOpacity={0.85}>
            <LogoutIcon size={18} color="#D94452" />
            <Text style={s.logoutText}>Log out</Text>
          </TouchableOpacity>

          <TouchableOpacity style={[s.deleteBtn, { backgroundColor: '#D94452' }]} onPress={handleDeleteAccount} activeOpacity={0.85}>
      ...
      {/* Delete modal */}
      <Modal visible={deleteModalVisible} transparent animationType="fade" onRequestClose={cancelDeleteAccount}>
        <View style={s.modalOverlay}>
          <View style={[s.modalCard, { backgroundColor: colors.card }]}>
            <Text style={[s.modalTitle, { color: colors.text }]}>Confirm Deletion</Text>
            <Text style={[s.modalDesc, { color: colors.muted }]}>Enter your password to permanently delete your account.</Text>
            <TextInput
              value={deletePassword} onChangeText={setDeletePassword}
              placeholder="Enter your password" placeholderTextColor={colors.caption}
            ...
            <View style={s.modalActions}>
              <TouchableOpacity style={[s.modalBtn, { backgroundColor: colors.surfaceAlt }]} onPress={cancelDeleteAccount} activeOpacity={0.8} disabled={deleting}>
                <Text style={[s.modalBtnText, { color: colors.muted }]}>Cancel</Text>
              </TouchableOpacity>
              <TouchableOpacity style={[s.modalBtn, { backgroundColor: '#D94452', opacity: deleting ? 0.6 : 1 }]} onPress={confirmDeleteAccount} activeOpacity={0.8} disabled={deleting}>
                <Text style={[s.modalBtnText, { color: '#FFFFFF' }]}>{deleting ? 'Deleting...' : 'Delete'}</Text>
              </TouchableOpacity>
            </View>
          </View>
        </View>
...
const s = StyleSheet.create({
  ...
  pageTitle: { fontFamily: 'Poppins_700Bold', fontSize: FontSize.xxl, marginBottom: Space.lg },

  section: { borderRadius: Radius.lg, marginBottom: Space.sm, ...CardShadow },
  sectionHeader: { flexDirection: 'row', alignItems: 'center', gap: 8, paddingHorizontal: Space.md, paddingTop: Space.md, paddingBottom: Space.sm, borderBottomWidth: 0.5 },
  ...
  sectionHeaderRow: { flexDirection: 'row', alignItems: 'center', gap: 8, marginBottom: Space.sm, marginTop: Space.sm },
  sectionLabel: { fontFamily: 'Nunito_700Bold', fontSize: FontSize.xs, letterSpacing: 0.3, textTransform: 'uppercase' },

  row: { flexDirection: 'row', alignItems: 'center', gap: Space.md, paddingHorizontal: Space.md, paddingVertical: 14, borderBottomWidth: 0.5, borderBottomColor: 'transparent' },
  ...
  themeChip: { paddingHorizontal: 12, paddingVertical: 6, borderRadius: Radius.pill },
  themeChipText: { fontFamily: 'Nunito_700Bold', fontSize: FontSize.xs },

  /* Destructive */
  logoutBtn: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8, borderRadius: Radius.lg, paddingVertical: 16, marginBottom: Space.sm, borderWidth: 1.5, ...CardShadow },
  logoutText: { fontFamily: 'Nunito_700Bold', fontSize: FontSize.base, color: '#D94452' },
  deleteBtn: { flexDirection: 'row', alignItems: 'center', justifyContent: 'center', gap: 8, borderRadius: Radius.lg, paddingVertical: 16, ...CardShadow },
  ...
  modalCard: { width: '100%', borderRadius: Radius.lg, padding: Space.lg, ...CardShadow },
  ...
  modalInput: { borderRadius: Radius.md, paddingHorizontal: Space.md, paddingVertical: 13, fontFamily: 'Nunito_400Regular', fontSize: FontSize.base, borderWidth: 1, marginBottom: Space.md },
  modalActions: { flexDirection: 'row', justifyContent: 'flex-end', gap: Space.sm },
  modalBtn: { paddingHorizontal: 20, paddingVertical: 10, borderRadius: Radius.pill },
  modalBtnText: { fontFamily: 'Nunito_700Bold', fontSize: FontSize.sm },

  /* Report styles */
  statsRow: { flexDirection: 'row', flexWrap: 'wrap', gap: Space.sm, marginBottom: Space.sm },
  emptyCard: { alignItems: 'center', paddingVertical: 40, borderRadius: Radius.lg, marginBottom: Space.sm, ...CardShadow },
  emptyText: { fontFamily: 'Nunito_400Regular', fontSize: FontSize.sm, marginTop: Space.sm },
  ...
  costCard: { borderRadius: Radius.lg, padding: Space.md, marginBottom: Space.sm, ...CardShadow },
  ...
  breakdownCard: { borderRadius: Radius.lg, padding: Space.md, marginBottom: Space.sm, ...CardShadow },
  breakdownTitle: { fontFamily: 'Poppins_600SemiBold', fontSize: FontSize.md, marginBottom: Space.md },
  barRow: { flexDirection: 'row', alignItems: 'center', marginBottom: Space.sm },
  barLabel: { fontFamily: 'Nunito_400Regular', fontSize: FontSize.xs, width: 80 },
  barTrack: { flex: 1, height: 10, borderRadius: 5, backgroundColor: 'rgba(0,0,0,0.06)', marginHorizontal: Space.sm, overflow: 'hidden' },
  ...
  recentCard: { borderRadius: Radius.lg, padding: Space.md, marginBottom: Space.sm, ...CardShadow },
  recentTitle: { fontFamily: 'Poppins_600SemiBold', fontSize: FontSize.md, marginBottom: Space.md },
  recentRow: { flexDirection: 'row', alignItems: 'flex-start', paddingVertical: Space.sm, borderBottomWidth: 0.5 },
...
const st = StyleSheet.create({
  statCard: { flex: 1, minWidth: '45%', borderRadius: Radius.md, padding: Space.sm, marginBottom: 0, ...CardShadow },
