...
function SummaryCard({ icon, val, label, colors, isLast }: SummaryCardProps) {
  return (
      <View
          style={[
            s.summaryCell,
      ...
      >
        <View style={s.summaryIconLine}>{icon}</View>
        <Text style={[s.summaryVal, { color: colors.text }]}>{val}</Text>
  ...
  return (
      <SafeAreaView style={[s.safe, { backgroundColor: colors.background }]} edges={['top']}>
        <ScrollView style={s.scroll} contentContainerStyle={s.content} showsVerticalScrollIndicator={false}>
          ...
          <View style={[s.summaryRow, { backgroundColor: colors.card }]}>
            <SummaryCard icon={<PackageIcon size={18} color={colors.cobalt} />} val={summary.active} label="On the way" colors={colors} />
            <SummaryCard icon={<AlertTriangleIcon size={18} color={colors.orange} />} val={summary.alerts} label="Need attention" colors={colors} />
            <SummaryCard icon={<CheckCircleIcon size={18} color={colors.green} />} val={summary.cleared} label="Cleared" colors={colors} isLast />
          </View>
